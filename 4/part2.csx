...
var boardsWon = new List<BingoBoard>();

for (int i = 0; i < numbersToCall.Count; i++)
{
    bool hasWon = false;
    WriteLine($"{i}: {boardsWon.Count} boards won out of {boards.Count}");
    ...
    foreach (BingoBoard board in boards)
    {
        ...
        {
            if (!boardsWon.Contains(board))
            {
                if (boardsWon.Count == boards.Count - 1)
                {
