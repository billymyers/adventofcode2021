...
int flashes = 0;
int iterations = 0;
bool allPointsFlashedThisStep = false;
...
while(!allPointsFlashedThisStep && iterations < 1000)
{
    RunIteration();
    iterations++;
}

WriteLine($"It took {iterations} iterations to find when all points flash");
void RunIteration()
{
    ...
    WriteLine($"Points flashed this iteration: {pointsFlashed.Count}");

    if(pointsFlashed.Count == (width * height))
        allPointsFlashedThisStep = true;
}
